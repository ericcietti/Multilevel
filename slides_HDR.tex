...
\begin{frame}{My contributions (I): extension to adaptive regularization  }


\begin{footnotesize}
\begin{tcolorbox}[
        title={\faExclamationTriangle\ Limitation},
    colback=red!10,
    colframe=red!70!black,
        fonttitle=\bfseries,
]
MG/OPT and RMTR have complicated convergence theory
...
\end{footnotesize}
	...
	\item Compute a step $s_k$: $\displaystyle{
	\min_{s\in\mathbb{R}^n} m_{k,\alert{q}}(s)=T_{k,\alert{q}}(s)+\frac{\lambda_k}{\alert{q+1}}\|s\|^{\alert{q+1}},\quad \lambda_k>0}
$
...
\vspace{-0.5cm}
	...
	\footnotesize{Notable example: \textcolor{myblue}{ARC} (adaptive cubic regularization \textcolor{myblue}{$q=2$})
		$$
...
2022]}}
		...
		\begin{frame}{Multilevel adaptive regularization}
		\begin{tcolorbox}[
		title=AR$q$: a better wort-case complexity,
    colback=gray!15,
    colframe=gray!50,
    arc=3mm,
    boxrule=0pt
		...
		\begin{itemize}
		\item TR: $\|\nabla f(x_k)\|\leq \epsilon$ requires $k=O(\epsilon^{\alert{-2}})$ 
		\item ARC: $\|\nabla f(x_k)\|\leq \epsilon$ requires $k=O(\epsilon^{\alert{-3/2}})$ 
		\item AR$q$: $\|\nabla f(x_k)\|\leq \epsilon$ requires $k=O(\epsilon^{\alert{-q+1/q}})$ 
		\end{itemize}
		\end{tcolorbox}	
		
		\begin{footnotesize}
\begin{tcolorbox}[
        title={\faExclamationTriangle\ Bottleneck},
    colback=red!10,
    colframe=red!70!black,
        fonttitle=\bfseries,
]
Subproblem solution: $\displaystyle \min_s m_{k,q}(s) $
...
\right)$, complexity ($O(\epsilon^{-(q+1)/q})$)
		\end{itemize}
...
\end{footnotesize}
 ...
 \includegraphics[scale=0.17]{figures/schema_new}
...
\begin{footnotesize}
\begin{itemize}
\item $L$ convex, differentiable and with $L_\Delta$-Lipschitz gradient
\item $S$ possibly non-smooth and non-proximable
\end{itemize}
\end{footnotesize}
\begin{tcolorbox}[
...
%	\end{small}
	...
	\begin{footnotesize}
\begin{tcolorbox}[
        title={\faExclamationTriangle\ Limitations  },
    colback=red!10,
    colframe=red!70!black,
        fonttitle=\bfseries,
]
\begin{itemize}
\item Do not allow for inexact prox evaluation
\item Do not use inertia
\end{itemize}
\end{tcolorbox}
\end{footnotesize}
...
\begin{frame}{Multilevel inexact inertial proximal method (IML FISTA)}
\vspace{-0.3cm}
\begin{columns}
\begin{column}{0.48\textwidth}
\begin{align*}
       x_{k+1} & \approx_\epsilon \prox_{\tau S}(\alert{{y}_{k} - \tau \nabla L ({y}_{k})}), \\
   y_{k+1} & =x_{k+1} + \alpha_{k}(x_{k+1}-x_{k})
\end{align*}

\end{column}
\begin{column}{0.48\textwidth}
\begin{align*}
    x_{k+1} & \approx_\epsilon \prox_{\tau S}(\alert{\text{ML}(y_k)}), \\
    y_{k+1} & = x_{k+1}+ \alpha_k(x_{k+1}-x_{k}) 
\end{align*}
\end{column}
...
\begin{figure}
\centering
\includegraphics[width=0.6\textwidth]{figures/ML_step_coarse}
\end{figure}

\begin{columns}
\begin{column}{0.7\textwidth}
 \begin{align*}
    \alert{ F_k^{H}}&= \Lo^H + \alert{{S^H_{\gamma_H}}} + \langle\textcolor{myblue}{v_k^H},\cdot\rangle \\
     \textcolor{myblue}{v_k^H}&=R\alert{\nabla f_{\gamma_h}^h}(x_k^{h})-\alert{\nabla f_{\gamma_H}^{H}} (Rx_{k}^{h})
\end{align*}
\begin{tcolorbox}[
      colback=white,
      colframe=gray!40,
...
SOTA convergence guarantees: \\$\left(f(x_k)-f(x^*)\right)=O\left(\frac{1}{k^2}\right)$\\
...
\begin{figure}
    \includegraphics[width = 0.6\textwidth]{images/moreauL1.eps}
    \caption{ Example: Moreau envelope of \textcolor{cyan}{$\ell_1$}-norm for \textcolor{orange}{$\gamma = 0.1$} and \textcolor{yellow}{$\gamma = 1$}}
    \end{figure}
\end{column}
\end{columns}
...
How to define $F_k^H, R, P$? 
\begin{itemize}
\item Hyperspectral images
...
\begin{frame}{Hyperspectral images}
...
$$
\widehat{x} \in\underset{x}{\arg\min}\; \underbrace{\frac{1}{2}\Vert A x - z \Vert_2^2}_{L} + \underbrace{\lambda \sum_i\| (D x)(i)\|_\star}_{S}
$$
...
$$
f^H(x^H):=\underbrace{\frac{1}{2}\Vert A^H x^H - R z \Vert_2^2}_{L^H} + \underbrace{\lambda \sum_i\| (D^H x)(i)\|_\star}_{S^H}
$$
\end{tcolorbox}
%\end{column}
%\end{columns}
...
\begin{frame}{Objective function evolution}
  ...
  \centering
\includegraphics[width=\textwidth]{figures/hyper_f}
...
\begin{frame}{Radio-interferometric imaging}
\centering
\includegraphics[width=\textwidth, trim=0 2cm 0 0, clip]{RI}
$$
\min_x f_i(x)=\frac{1}{2}\|A x-z\|^2+\|W_iD^*x\|_1+i_{\mathbb{R}^n_+}(x)
$$
...
\centering
\includegraphics[width=\textwidth]{figures/RI_new}%RI6
...
\includegraphics[scale=0.15]{figures/proj_learning3}
	...
	\begin{frame}{(1) Physics informed neural networks (PINNs) training}
...
\begin{table}
\begin{tabular}{lc}
Find $\theta$ such that  $ h_z(\theta)\approx u(z)$ for all $z\in\Omega$ & Training data\\

    \includegraphics[scale=0.15]{figures/pinn_schema2}&
...
\begin{footnotesize}
\begin{align*}
 { \color{red} L_{OBS}(\theta)} & {\small =\gamma_1\sum_{z_i\in\Omega \cup  \partial \Omega} (h_\theta(z_i)-y_i)^2},  \\ 
  {\color{orange} L_{PDE}(\theta)}&{\small=\gamma_{2}\sum_{z_i\in \Omega}(\mathcal{L}(h_{z_i}(\theta))-\phi(z_i))^2}
  \\
  {\color{blue} L_{BC}(\theta)}&{\small =\gamma_{3} \sum_{z_i\in \partial\Omega}(\mathcal{B}(h_{z_i}(\theta))-\psi(z_i))^2}
\end{align*}
   \end{footnotesize}
...
\begin{figure}
\includegraphics[width=\textwidth]{images/spectral_bias.pdf}
%  \caption{
...
\begin{frame}{Numerical results: final MSE on average (10 runs)}
 Problem: Poisson (left) and heat problem (right)
\begin{figure}
...
\begin{frame}{(2) Expected risk minimization }
 $$
      \min_{x \in \mathbb{R}^n} \mathbb{E}_P[\mathcal{L}(m_x(z),y)]
$$
...
\end{figure}
	 ...
	 \small{ \textcolor{mygreen}{[  Marini, Porcelli, R., MathComp, 2026]}}
	\end{frame}
...
Given a dense matrix $A$, find \emph{sparse} factors $S_1, S_2, \ldots, S_J$ such that: 
...
\begin{itemize}		
	 ...
	 $A = XY^\top$, $A$ data, $X$  dictionary,  $Y$  representation of samples in the dictionary.
\end{itemize}		
\begin{tiny} 
				\textcolor{gray}{[S. Foucart, H. Rauhut,  2013]}
				\end{tiny}
	...
	\item Sparse (linear) neural networks 
		...
		\begin{figure}
			\centering
			\includegraphics[width=0.7\textwidth]{sparse-NN.pdf}
		\end{figure}
		\end{columns}
		
				\begin{tiny} 
				\textcolor{gray}{[T. Dao $\&$ all., 2019]	
					[B. Chen $\&$ all.,  2022]	}		\end{tiny}
...
%\section{Fixed support matrix factorization (FSMF)}
%\frame{\tableofcontents[currentsection]}

%\subsection{A general formulation for sparse matrix factorization}
%\frame{\tableofcontents[currentsubsection, hideothersubsections, sectionstyle=show/shaded, subsectionstyle=show/shaded,]}

\begin{frame}{A general formulation for sparse matrix factorization}
...
\begin{tcolorbox}[title=\textbf{Two} factors matrix factorization]

 
Given $A$
				$	\min_{X, Y} \; \|A - XY^\top\|_F^2$  subject to: $X, Y$ sparse matrices
	\end{tcolorbox}
...
\begin{frame}{A classical related problem: sparse linear inverse problem}
	\begin{tcolorbox}[top=0pt,bottom=0pt]
		\setlength{\abovedisplayskip}{1pt}
		\begin{equation*}
		\hspace{-1cm}\min_{y \in \mathbb{R}^n} \|a - Xy\|^2_2 \quad \text{subject to: }   \|y\|_0 \leq s , s \ll n 
		\end{equation*}
	\end{tcolorbox}
	\centering
	  \includegraphics[width=0.4\textwidth]{undetermined}

	
	  
  \includegraphics[width=0.4\textwidth]{signal}
  \includegraphics[width=0.4\textwidth]{sparse_signal}
  \vspace{0.5cm}
  
  \includegraphics[width=\textwidth]{wavelets_transf}
...
\begin{frame}{A classical related problem: sparse linear inverse problem}
	\begin{tcolorbox}[top=0pt,bottom=0pt]
		\setlength{\abovedisplayskip}{1pt}
		\begin{equation*}
		\hspace{-1cm}\text{Given } a\in\mathbb{R}^m, X\in\mathbb{R}^{m\times n},\min_{y \in \mathbb{R}^n} \|a - Xy\|^2_2 \quad \text{subject to: }   \|y\|_0 \leq s , s \ll n 
		\end{equation*}
	\end{tcolorbox}
\pause
	\begin{tcolorbox}[title = \text{1) Support identification}, colframe = red!50!yellow, colbacktitle = red!50!yellow]
		Finding a set $I \subseteq \intset{n}$ such that $|I| = s$.
	\end{tcolorbox}
	\only<1>{\vspace{90pt}}
	\only<2>{
		\begin{tcolorbox}[title = \text{2) Optimize coefficients inside support}, colframe = red!50!yellow, colbacktitle = red!50!yellow]
			\setlength{\abovedisplayskip}{0pt}
			\begin{equation*}
				\label{eq: fixed_supp_prob2}
				\underset{y \in \mb{R}^n, \supp(y) \subseteq I}{\text{Minimize}} 
				\|a - Xy\|^2_2\\
			\end{equation*}
		\end{tcolorbox}
	}
	...
	\only<3>{
		\begin{tcolorbox}[title = \text{2) Optimize coefficients inside support}, colframe = red!50!yellow, colbacktitle = red!50!yellow]
			...
			\begin{equation*}
					\underset{y \in \mb{R}^n, \supp(y) \subseteq I}{\text{Minimize}} 
				\|a  - Xy\|^2_2 \textcolor{green}{= \|a - X_{I}y_{I}\|^2_2}\\
			\end{equation*}
		\end{tcolorbox}
	}
	
	\only<4->{
		\begin{tcolorbox}[title = \text{2) \emph{Linear regression problem}}, colbacktitle =  green, colframe= green]
			\setlength{\abovedisplayskip}{0pt}
			\begin{equation*}
							\underset{\tilde{y} \in \mb{R}^{|I|}}{\text{Minimize}} \quad
				\|a - X_{I}\tilde{y}\|^2_2\\
			\end{equation*}
		\end{tcolorbox}
	}
	\onslide<3->{
		...
		\begin{figure}
			\centering
			\includegraphics[scale = 0.25]{sparseprod.png}
		\end{figure}
	}
\end{frame}
...
%\subsection{Fixed support matrix factorization - formulation and questions}
%\frame{\tableofcontents[currentsubsection, hideothersubsections, sectionstyle=show/shaded, subsectionstyle=show/shaded,]}

\begin{frame}{Two sub-problems of \textbf{two} factors matrix factorization}
	\begin{tcolorbox}[top=0pt,bottom=0pt]
		\setlength{\abovedisplayskip}{0pt}
		\begin{equation*}
			\label{eq:sparse_recovery1}
	...
	\pause
	\begin{tcolorbox}[title = \text{1) Support identification}, colframe =  red!50!yellow, colbacktitle= red!50!yellow]
		Choose \emph{two} sets $S_X \subseteq \intset{m} \times \intset{r}$ and $S_Y \subseteq \intset{n} \times \intset{r}$ satisfying  $\mc{E}$ 	\end{tcolorbox}
	\only<1-2>{\vspace{90pt}}
	\only<3>{
		\begin{tcolorbox}[title = \text{2) Optimize coefficients inside support}, colframe =  red!50!yellow, colbacktitle= red!50!yellow]
			\setlength{\abovedisplayskip}{0pt}
			\begin{equation*}
...
\begin{frame}{Two sub-problems of \textbf{two} factors matrix factorization}
	...
	\begin{tcolorbox}[title = \text{1) Support identification}, colframe =  red!50!yellow, colbacktitle= red!50!yellow]
		Find \emph{two} sets $S_X \subseteq \intset{m} \times \intset{r}$ and $S_Y \subseteq \intset{n} \times \intset{r}$ satisfying  $\mc{E}$ 	\end{tcolorbox}
%	\only<1-2>{\vspace{90pt}}
...
\end{frame}
	...
	\begin{columns}
		...
		\begin{figure}[h]
			\centering
		\includegraphics[scale=0.22]{hodlr_git}
		\end{figure}
		\end{column}
		\end{columns}
...
\vfill
		...
		\item There is a family of \emph{polynomially solvable} instances and  an  \emph{efficient algorithm} to solve them
	...
	\end{enumerate}
		 ...
		 \small{ \textcolor{mygreen}{[Zheng, R., Gribonval, SIMODS, 2023]}}, 
 \small{ \textcolor{mygreen}{[Le, R., Gribonval, SIMAX, 2023] }}
...
\begin{frame}{My contributions}
		...
		\begin{enumerate}
				\item The problem is  \emph{NP-hard}.
		\vfill
...
\vfill
		...
		\item  \fbox{\parbox{0.85\textwidth}{There is a family of \emph{polynomially solvable} instances and  an  \emph{efficient algorithm} to solve them}}
		
		\vfill
		\item Some properties of the \emph{landscape} of the function $L(X,Y) = \|A - XY^\top\|^2$ \emph{under the support constraints} are known, which help to understand how well \textcolor{red}{gradient descent} tackles the problem of FSMF
...
\begin{frame}{SVD as a greedy algorithm: the constrained case}

    \begin{itemize}

        \item How to generalize the greedy algorithm?
...
\begin{column}{0.5\textwidth}
 ...
 \end{tcolorbox}}
		 ...
		 \small{ \textcolor{mygreen}{[Le, Zheng, R., Gribonval, ICASSP, 2022 and SIMAX, 2025]}}
...
\begin{center}
  \only<1 >{\includegraphics[width=0.7\textwidth]{figures/results_0}}%
  \only<2 >{\includegraphics[width=0.7\textwidth]{figures/results_1}}%
  \only<3->{\includegraphics[width=0.7\textwidth]{figures/results_2}}%
\end{center}
\begin{itemize}
...
\begin{frame}{Floating-point arithmetic}
      ...
      \begin{tcolorbox}[
      title=Floating-point arithmetic model,
      colback=white,
      colframe=gray!40,
      boxrule=0.4pt,
      arc=4pt,
...
\begin{column}{0.5\textwidth}
  Low precisions 
  \begin{itemize}
   \item[\textcolor{green}{\faSmile}]  Reduction of energy and increased speed 
     \item[\textcolor{red}{\faFrown}]      Loss of accuracy
  \end{itemize}
  \end{column}
  \begin{column}{0.5\textwidth}
  $\rightarrow$ Mixed precision
  \begin{itemize}
  \item Forward pass of neural networks \small{ \textcolor{mygreen}{[El Arar, Filip, Mary, R., IMAJNA, 2026]}}
...
\begin{tcolorbox}{Error model}
  \begin{align*}
      &\widehat{d}_i := - \big(H(\widehat{x}_i) + \tikz[remember picture, baseline=(EH.base)]{\node[inner sep=1pt] (EH) {$E^H_i$};}\big)^{-1}\big(g(\widehat{x}_i) + \tikz[remember picture, baseline=(Eg.base)]{\node[inner sep=1pt] (Eg) {$e^g_i$};}\big), \\
      &\widehat{x}_{i+1} =  \widehat{x}_i + \widehat{d}_i + \tikz[remember picture, baseline=(Ep.base)]{\node[inner sep=1pt] (Ep) {$e^+_i$};}.
  \end{align*}

  \begin{itemize}
    \item<2-> \textcolor{myblue!70!black}{$E^H_i$: Hessian approximations/inexact linear solvers.}
    \item<3-> \textcolor{gray!70!black}{$e^g_i$: inexact gradient computations.}
    \item<4-> \textcolor{myyel!70!black}{$e^+_i$: errors in the update step.}
  \end{itemize}

  \begin{tikzpicture}[overlay, remember picture]
    \only<2->{
      \draw[rounded corners=3pt, thick, color=myblue!70!black, fill=myblue!10, fill opacity=0.3]
        ([xshift=-2pt, yshift=2pt]EH.north west) rectangle
        ([xshift=2pt,  yshift=-2pt]EH.south east);
    }
    \only<3->{
      \draw[rounded corners=3pt, thick, color=gray!70!black, fill=gray!10, fill opacity=0.3]
        ([xshift=-2pt, yshift=2pt]Eg.north west) rectangle
        ([xshift=2pt,  yshift=-2pt]Eg.south east);
    }
    \only<4->{
      \draw[rounded corners=3pt, thick, color=myyel!70!black, fill=myyel!10, fill opacity=0.3]
        ([xshift=-2pt, yshift=2pt]Ep.north west) rectangle
        ([xshift=2pt,  yshift=-2pt]Ep.south east);
    }
  \end{tikzpicture}
  \end{tcolorbox}
\end{frame}
...
\begin{frame}{Local convergence analysis}
  ...
  \begin{equation*}
      ...
      \tikz[remember picture, baseline=(alpha.base)]{\node[inner sep=1pt] (alpha) {$\alpha_i$};}\norm{\widehat{x}_i - x^*}^2
      + \tikz[remember picture, baseline=(beta.base)]{\node[inner sep=1pt] (beta) {$\beta_i$};}\norm{ \widehat{x}_i -x^* }
      + \tikz[remember picture, baseline=(gamma.base)]{\node[inner sep=1pt] (gamma) {$\limacc$};},
    \end{equation*}
    \begin{center}
        \usebeamerfont{caption}\usebeamercolor[fg]{caption}\emph{Mixed precision Newton's method convergence rate.}
      \end{center}
    \end{tcolorbox}
    \begin{itemize}
      \item<2-> $\alpha_i$: standard Newton's quadratic convergence;
      \item<3-> \textcolor{myblue!70!black}{$\beta_i$: linear convergence influenced by Hessian errors;}
      \item<4-> \textcolor{gray!70!black}{$\gamma_i$: \textbf{limiting accuracy} mainly related to gradient errors.}
    \end{itemize}

  \begin{tikzpicture}[overlay, remember picture]
    \only<2->{
      \draw[rounded corners=3pt, thick, color=black, fill=black!10, fill opacity=0.3]
        ([xshift=-2pt, yshift=2pt]alpha.north west) rectangle
        ([xshift=2pt,  yshift=-2pt]alpha.south east);
    }
    \only<3->{
      \draw[rounded corners=3pt, thick, color=myblue!70!black, fill=myblue!10, fill opacity=0.3]
        ([xshift=-2pt, yshift=2pt]beta.north west) rectangle
        ([xshift=2pt,  yshift=-2pt]beta.south east);
    }
    \only<4->{
      \draw[rounded corners=3pt, thick, color=gray!70!black, fill=gray!10, fill opacity=0.3]
        ([xshift=-2pt, yshift=2pt]gamma.north west) rectangle
        ([xshift=2pt,  yshift=-2pt]gamma.south east);
    }
  \end{tikzpicture}
  ...
  \begin{tcolorbox}[
  title=Floating point Newton's method,
      colback=white,
      colframe=gray!40,
      boxrule=0.4pt,
      arc=4pt,
...
\begin{frame}{Floating point inexact Newton's methods}
\begin{tcolorbox}[
 title={Inexact Newton},
    ...
    colframe=red!70!black,
        fonttitle=\bfseries,
         ...
         $
\norm{H(\widehat{x}_i)\widehat{d}_i+g(\widehat{x}_i)}\leq \eta_i \norm{g(\widehat{x}_i)}.
$
\end{tcolorbox}

Satisfies model with: 
$$
\eh = c_{\text{solver}}u_H+\eta_i \frac{\norm{g(\widehat{x}_i)}}{\norm{H(\widehat{x}_i)}\norm{\widehat{d}_i)}} %=: \frac{\eta_i}{\zeta_i}
$$
%where $\zeta_i = \norm{H(\widehat{x}_i)}\norm{\widehat{d}_i}/\norm{g(\widehat{x}_i)}$
\centering
...
\begin{frame}{Some perspectives }
    \includegraphics[width=\linewidth]{figures/perspectives3.pdf}
